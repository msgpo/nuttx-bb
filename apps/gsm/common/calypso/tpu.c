...
static void tpu_ram_read_en(int enable)
{
	...
	reg = getreg16(TPU_REG(TPU_CTRL));
	if (enable)
		reg |= TPU_CTRL_MCU_RAM_ACC;
	else
		reg &= ~TPU_CTRL_MCU_RAM_ACC;
	putreg16(reg, TPU_REG(TPU_CTRL));
}
...
static int tpu_wait_ctrl_bit(uint16_t bit, int set)
{
	...
	while (1) {
		uint16_t reg = getreg16(TPU_REG(TPU_CTRL));
		if (set) {
			if (reg & bit)
				break;
		} else {
...
void tpu_wait_idle(void)
{
	...
	while (getreg16(TPU_REG(TPU_CTRL)) & TPU_CTRL_IDLE)
		dputchar('.');
	dputs("Done!\n");
}
...
{
	uint8_t reg = getreg8(TPU_REG(INT_CTRL));
	if (mcu)
		reg &= ~ICTRL_MCU_FRAME;
	else
		reg |= ICTRL_MCU_FRAME;
...
{
	return getreg16(TPU_REG(TPU_OFFSET));
}

uint16_t tpu_get_synchro(void)
{
	return getreg16(TPU_REG(TPU_SYNCHRO));
}
