...
{
	while (getreg16(BL_CMD_STATUS) != BL_STATUS_IDLE);
}

static void dsp_bl_start_at(uint16_t addr)
{
	putreg16(0, BL_ADDR_HI);
	putreg16(addr, BL_ADDR_LO);
	putreg16(0, BL_SIZE);
	putreg16(BL_CMD_COPY_BLOCK, BL_CMD_STATUS);
}
...
static int dsp_bl_upload_sections(const struct dsp_section *sec)
{
	...
	/* Set mode */
	putreg16(BL_MODE_DATA_WRITE, BASE_API_RAM);
	putreg16(BL_CMD_COPY_MODE, BL_CMD_STATUS);
	dsp_bl_wait_ready();
	...
	for (; sec->data; sec++) {
		...
		/* Issue DRAM write */
		putreg16(sec->addr >> 16, BL_ADDR_HI);
		putreg16(sec->addr & 0xffff, BL_ADDR_LO);
		putreg16(sec->size, BL_SIZE);
		putreg16(BL_CMD_COPY_BLOCK, BL_CMD_STATUS);
...
static void dsp_pre_boot(const struct dsp_section *bootcode)
{
	...
	if (bootcode) {
		...
		putreg16(BL_STATUS_NA, BL_CMD_STATUS);
	} else
		delay_ms(10);
...
static void wait_for_frame_irq(void)
{
	...
	while (getreg8((void *)0xffff1000) & (1<<4))
	;//	putchar('.');
	//puts("Done!\n");
}
...
static void _dsp_dump_range(uint32_t addr, uint32_t size, int mode)
{
	...
	while (size) {
		...
		putreg16(addr >> 16, BL_ADDR_HI);
		putreg16(addr & 0xffff, BL_ADDR_LO);
		putreg16(bs, BL_SIZE);
		putreg16(BL_CMD_COPY_BLOCK, BL_CMD_STATUS);
...
void dsp_dump(void)
{
	...
	putreg16(0xffff, BL_CMD_STATUS);
	dsp_bl_wait_ready();
	printf("DSP bootloader version 0x%04x\n", getreg16(BASE_API_RAM));
