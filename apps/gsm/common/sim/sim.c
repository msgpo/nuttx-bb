...
#else
#include "../../../nuttx/arch/arm/src/common/up_arch.h"
#include "../../../nuttx/include/nuttx/arch.h"
#include <string.h>
#include <abb/twl3025.h>
...
#define SIM_DEBUG_OUTPUTDELAY 200000
	...
	regVal = getreg16(REG_SIM_DTX);
	printf("  |-REG_SIM_DTX = %02x (next data byte to be transmitted)\n", regVal);
	usleep(SIM_DEBUG_OUTPUTDELAY);

	regVal = getreg16(REG_SIM_MASKIT);
...
{
	printf("Triggering SIM reception\n");
	...
	putreg16(getreg16(REG_SIM_CONF1) & ~REG_SIM_CONF1_CONFTXRX, REG_SIM_CONF1);

	/* Unmask the interrupts that are needed to perform this action */
	putreg16(~(REG_SIM_MASKIT_MASK_SIM_RX | REG_SIM_MASKIT_MASK_SIM_WT),
		REG_SIM_MASKIT);
...
{
	printf("Triggering SIM transmission\n");
	...
	/* Unmask the interrupts that are needed to perform this action */
	putreg16(~(REG_SIM_MASKIT_MASK_SIM_TX), REG_SIM_MASKIT);

	/* Transmit the first byte manually to start the interrupt cascade */
	putreg16(*tx_buffer,REG_SIM_DTX);
	tx_buffer++;
...
void sim_irq_handler(int irq, uint32_t *regs)
{
	int regVal = getreg16(REG_SIM_IT);
...
//				usleep(1);
				/* Switch I/O direction to input */
				putreg16(getreg16(REG_SIM_CONF1) &
	...
	if(regVal & REG_SIM_IT_SIM_RX) {
		uint8_t ch = (uint8_t) (getreg16(REG_SIM_DRX) & 0xFF);

		/* ignore NULL procedure byte */
		if(ch == 0x60 && sim_ignore_waiting_char) {
			printf(" 0x60 received...\n");
			return;
		}
		...
		*rx_buffer = ch;
		...
		if (sim_rx_max_character_count
		 && sim_rx_character_count >= sim_rx_max_character_count) {
			printf(" Max characters received!\n");
			rxDoneFlag = 1;
		}
	}
}
...
void calypso_sim_init(void)
{
	...
	printf("SIM: Registering interrupt handler for simcard-interface\n");

	irq_attach(IRQ_SIMCARD, (xcpt_t)sim_irq_handler);

	up_prioritize_irq(IRQ_SIMCARD, 0xff);

	up_enable_irq(IRQ_SIMCARD);
}
...
int calypso_sim_powerup(uint8_t *atr)
{
	...
	usleep(SIM_OPERATION_DELAY);

	/* Enable clock */
	putreg16(REG_SIM_CMD_MODULE_CLK_EN | REG_SIM_CMD_CMDSTART, REG_SIM_CMD);
	printf(" * Clock enabled!\n");
	usleep(SIM_OPERATION_DELAY);

	/* Release reset */
	putreg16(getreg16(REG_SIM_CONF1) | REG_SIM_CONF1_CONFBYPASS
...
{
	putreg16(getreg16(REG_SIM_CONF1) & ~REG_SIM_CONF1_CONFBYPASS, REG_SIM_CONF1);
	printf(" * Reset pulled down!\n");
	usleep(SIM_OPERATION_DELAY);

	putreg16(REG_SIM_CMD_MODULE_CLK_EN | REG_SIM_CMD_CMDSTOP, REG_SIM_CMD);
	printf(" * Clock disabled!\n");
	usleep(SIM_OPERATION_DELAY);

	putreg16(0, REG_SIM_CMD);
...
int calypso_sim_reset(uint8_t *atr)
{
	...
	/* Pull reset down */
	putreg16(getreg16(REG_SIM_CONF1) & ~REG_SIM_CONF1_CONFSRSTLEV,
		REG_SIM_CONF1);
	printf(" * Reset pulled down!\n");
	...
	/* Pull reset down */
	putreg16(getreg16(REG_SIM_CONF1) | REG_SIM_CONF1_CONFSRSTLEV, REG_SIM_CONF1);
	printf(" * Reset released!\n");
